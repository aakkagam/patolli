 ...
 */
const CARD_PIECES = [
  // All on plain squares, so no counter hides a wedge, a rounded tip or an
  // entry mark. 2 and 3 are adjacent: the blocking that is the whole game.
  { index: 2, player: 0 },
  { index: 3, player: 0 },
  { index: 20, player: 1 },
  { index: 41, player: 1 },
 ...
 */
function renderIcon(geometry, tokens) {
  const { CELL, VIEW } = geometry;
  const mid = 32; // centre of the icon's 64-unit viewBox

  /**
   * Proportions come from the board, so a change to the geometry reaches the
   * icon too. The board's arm is one cell either side of centre out of a
   * half-extent of eight, and its arms run to the very edge.
   */
  const armRatio = CELL / (VIEW.width / 2);

  /**
   * The one deliberate departure. At the board's true proportion the icon's
   * arms would be four pixels wide at 32px, which is the thinness that made
   * the first attempt vanish. Doubling keeps the silhouette a cross while
   * giving it enough mass to survive a tab bar; it is the legibility decision
   * `design.md` argues for, made explicit rather than baked into a literal.
   */
  const BOLDEN = 2;
  const half = mid * armRatio * BOLDEN;
  const arm = mid * 0.78; // reach to the tip, inset so the mark is not flush
  ...
  <rect x="${mid - half}" y="${mid - half}" width="${half * 2}" height="${half * 2}" rx="1.5" fill="${cochineal}" />
</svg>
`;
}
...
try {
  ...
  writeFileSync(resolve(root, 'public/icon.svg'), renderIcon(geometry, tokens));
